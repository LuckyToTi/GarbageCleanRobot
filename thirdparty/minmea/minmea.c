...
enum minmea_sentence_id minmea_sentence_id(const char *sentence, bool strict)
{
    ...
    if (!strcmp(type, "DVEXT")){
        return MINMEA_SENTENCE_DVEXT;
    }
    if (!strcmp(type, "DVPDL")){
        return MINMEA_SENTENCE_DVPDL;
    }
...
bool minmea_parse_dvext(struct minmea_sentence_dvext *frame, const char *sentence){
    char type[6];
    if (!minmea_scan(sentence, "tccifffffffffffffffiiiiccccffffffff",  // 格式化串
            type,  // Type ID
            &frame->v, &frame->g, &frame->abcd, // v, g, abcd
            &frame->r.value, &frame->p.value, &frame->h.value, &frame->k, &frame->u.value,  // r, p, h, k, u
            &frame->t, &frame->n, &frame->e, &frame->lat, &frame->long_e, &frame->eee,  // t, n, e, lat, long_e, eee
            &frame->qw, &frame->qx, &frame->qy, &frame->qz, &frame->ga, &frame->gb, &frame->gc, &frame->gd,  // qw, qx, qy, qz, ga, gb, gc, gd
            &frame->la, &frame->lb, &frame->lc, &frame->ld, &frame->va, &frame->vb, &frame->vc, &frame->vd,  // la, lb, lc, ld, va, vb, vc, vd
            &frame->ra, &frame->rb, &frame->rc, &frame->rd  // ra, rb, rc, rd
    )) {
        return false;  // 解析失败
    }

    if (strcmp(type, "DVEXT") != 0) {
        return false;
    }
    return true;  // 解析成功
}

bool minmea_parse_dvpdl(struct minmea_sentence_dvpdl *frame, const char *sentence){
    char type[6];
    if (!minmea_scan(sentence, "tTdffffffd",  // 格式化串
            type,  // Type ID
            &frame->tu,
            &frame->dtu,
            &frame->adr, &frame->adp, &frame->ady,
            &frame->pdx, &frame->pdy, &frame->pdz,
            &frame->c
    )) {
        return false;  // 解析失败
    }

    if (strcmp(type, "DVPVL") != 0) {
        return false;
    }
    return true;  // 解析成功
}
